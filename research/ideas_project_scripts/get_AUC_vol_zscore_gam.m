...
mdl = fitrgam(control_data,'Left_Hippocampus ~ Binned_Age_at_Scan + Sex + EstimatedTotalIntraCranialVol',CategoricalPredictors='Sex');
data.vol_LR = data.Left_Hippocampus-predict(mdl,data);

mdl = fitrgam(control_data,'Right_Hippocampus ~ Binned_Age_at_Scan + Sex + EstimatedTotalIntraCranialVol',CategoricalPredictors='Sex');
data.vol_RR = data.Right_Hippocampus-predict(mdl,data);

%% get z-score for patients
data.z_vol_L = (data.vol_LR-mean(data.vol_LR(data.Disease == "Controls")))/std(data.vol_LR(data.Disease == "Controls"));
data.z_vol_R = (data.vol_RR-mean(data.vol_RR(data.Disease == "Controls")))/std(data.vol_RR(data.Disease == "Controls"));
...
%%
stats = compute_roc(L_HS,data.z_vol_L,"false");
fprintf('Left HS \n')
fprintf('AUC (CI): %.2f (%.2f - %.2f) \n',stats(3),stats(4),stats(5))
ci = bootci(1000,@(x,y,z) compute_sensspec(x,y,z),L_HS,data.z_vol_L,0);
...
stats = compute_cv(L_HS,data.z_vol_L,"false");
...
%% 
stats = compute_roc(R_HS,data.z_vol_R,"false");
fprintf('Right HS \n')
fprintf('AUC (CI): %.2f (%.2f - %.2f) \n',stats(3),stats(4),stats(5))
ci = bootci(1000,@(x,y,z) compute_sensspec(x,y,z),R_HS,data.z_vol_R,0);
fprintf('Sensitivity (CI): %.2f (%.2f - %.2f)\n',stats(1),ci(1,1),ci(2,1))
fprintf('Specificity (CI): %.2f (%.2f - %.2f)\n',stats(2),ci(1,2),ci(2,2))
fprintf('Overall Threshold: %.3f\n',stats(6))
stats = compute_cv(R_HS,data.z_vol_R,"false");
fprintf('Mean Threshold (5 fold CV): %.3f\n',stats(1));
fprintf('Range Threshold (5 fold CV): %.3f to %.3f \n',stats(2),stats(3));